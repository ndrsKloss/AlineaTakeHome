...
struct AlineaKeyboard: View {
    ...
    /// the fixed row pitch (empirically verified). Standard sizes and the lower
    /// accessibility sizes still scale (`NFR-A11Y-001/002`). Exposed (not
    /// `private`) so it can be asserted in tests.
